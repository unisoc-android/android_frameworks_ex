...
public class SprdAndroidCamera2Capabilities extends CameraCapabilities {
    ...
    SprdAndroidCamera2Capabilities(Stringifier stringifier, CameraCharacteristics p) {
        ...
        buildZoomRatioSection(p);
        buildLogoWatermarkEnable(p);
        buildTimeWatermarkEnable(p);
        ...
        mExposureTimeRange = p.get(SENSOR_INFO_EXPOSURE_TIME_RANGE);
        buildExposureTimeList(p);
        mMinFocusDistance = p.get(LENS_INFO_MINIMUM_FOCUS_DISTANCE);
    }

    private void buildExposureTimeList(CameraCharacteristics p) {
        long unit = 1000000000L;
        final long SENSOR_INFO_EXPOSURE_TIME_RANGE_MIN_SHOW = 1000000L;// unit ns
        final long SENSOR_INFO_EXPOSURE_TIME_RANGE_MAX_SHOW = 200000000L;
        Range<Long> timeRange = p.get(SENSOR_INFO_EXPOSURE_TIME_RANGE);
        long minValue = timeRange.getLower();
        long maxValue = timeRange.getUpper();
        long[] exposureDNList = new long[] {
                1000, 500, 250, 125, 60, 30, 15, 8, 5
        };

        //Log.i(TAG, "buildExposureTimeList minValue=" + minValue + ", maxValue=" + maxValue);
        if (minValue < SENSOR_INFO_EXPOSURE_TIME_RANGE_MIN_SHOW) {
            minValue = SENSOR_INFO_EXPOSURE_TIME_RANGE_MIN_SHOW;
        }
        if (maxValue > SENSOR_INFO_EXPOSURE_TIME_RANGE_MAX_SHOW) {
            maxValue = SENSOR_INFO_EXPOSURE_TIME_RANGE_MAX_SHOW;
        }
        //Log.i(TAG, "buildExposureTimeList 2 minValue=" + minValue + ", maxValue=" + maxValue);
        // minValue=1000000, maxValue=200000000 ns
        int i = 0;
        while (minValue > (1.0 / exposureDNList[i]) * 1e9) {
            if (i++ >= exposureDNList.length) {
                break;
            }
        }

        int j = exposureDNList.length - 1;
        while (maxValue < (1.0 / exposureDNList[i]) * 1e9) {
            if (j-- <= 0) {
                break;
            }
        }
        //Log.i(TAG, "buildExposureTimeList i=" + i + ", j=" + j);
        mExposureTimeList.add((long) 0);
        if (i < exposureDNList.length && j > 0) {
            for (int k = i; k <= j; k++) {
                mExposureTimeList.add(exposureDNList[k]);
            }
        }
    ...
    private void buildFaceAttributesEnable(CameraCharacteristics p){
        try {
            if(p.get(CONTROL_AVAILABLE_FACEFEATUREENABLE) == null){
                ...
                byte supportedFaceAttributesEnable = p.get(CONTROL_AVAILABLE_FACEFEATUREENABLE);
    ...
    private void buildZoomRatioSection(CameraCharacteristics p){
        float[] sprdZoomRatioSection = p.get(ANDROID_SPRD_ZOOM_RATIO_SECTION);
        Log.i(TAG, "mSupportedAiScene = " + sprdZoomRatioSection);
        if (sprdZoomRatioSection != null)
            mSprdZoomRatioSection = sprdZoomRatioSection;
    }
    private void buildLogoWatermarkEnable(CameraCharacteristics p) {
        int supportedLogoWatermark = p.get(CONTROL_AVAILABLE_LOGOWATERMARK);
        Log.i(TAG, "mSupportedLogoWatermark = " + supportedLogoWatermark);
        if (1 == supportedLogoWatermark)
            mSupportedLogoWatermark = true;
    }
    private void buildTimeWatermarkEnable(CameraCharacteristics p) {
        int supportedTimeWatermark = p.get(CONTROL_AVAILABLE_TIMEWATERMARK);
        Log.i(TAG, "mSupportedTimeWatermark = " + supportedTimeWatermark);
        if (1 == supportedTimeWatermark)
            mSupportedTimeWatermark = true;
    }
}
