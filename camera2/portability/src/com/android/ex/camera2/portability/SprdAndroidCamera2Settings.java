...
public class SprdAndroidCamera2Settings extends AndroidCamera2Settings {
    ...
    public static final Key<Integer> CONTROL_SPRD_SMILE_CAPTURE = new Key<Integer>(
            "com.addParameters.sprdSmileCapture", int.class);
    ...
    public Camera2RequestSettingsSet getRequestSettings() {
        ...
        if(mCurrentEnableLogoWatermark != -1){
            Log.i(TAG, " set ANDROID_SPRD_ENABLED: mCurrentEnableLogoWatermark = "
                    + mCurrentEnableLogoWatermark);
            requestSettings.set(ANDROID_SPRD_LOGOWATERMARK_ENABLED, mCurrentEnableLogoWatermark);
        }
        if(mCurrentEnableTimeWatermark != -1){
            Log.i(TAG, " set ANDROID_SPRD_ENABLED: mCurrentEnableTimeWatermark = "
                    + mCurrentEnableTimeWatermark);
            requestSettings.set(ANDROID_SPRD_TIMEWATERMARK_ENABLED, mCurrentEnableTimeWatermark);
        }
        // add tag for chuanyin 3dnr and auto3dnr
        ...
        if (mCurrentEnableFaceAttributes != -1) {
            Log.i(TAG, " set ANDROID_SPRD_FACEATTRIBUTES_ENABLED: mCurrentEnableFaceAttributes = " + mCurrentEnableFaceAttributes);
            requestSettings.set(ANDROID_SPRD_FACEATTRIBUTES_ENABLED, mCurrentEnableFaceAttributes);
        }

        if (mTouchAreas.size()>0){
            requestSettings.set(CONTROL_SPRD_TOUCHINFO_REGION,legacyAreasToMeteringRectangles(mTouchAreas));
        }else{
            MeteringRectangle[] rect = new MeteringRectangle[1];
            rect[0] = new MeteringRectangle(0,0,0,0,0);
            requestSettings.set(CONTROL_SPRD_TOUCHINFO_REGION,rect);
        }
        /* @} */

        if(mCurrentEnableAutoChasing != -1){
            Log.i(TAG, " set ANDROID_SPRD_AUTO_CHASING_ENABLED: mCurrentEnableAutoChasing = "
                    + mCurrentEnableAutoChasing);
            requestSettings.set(ANDROID_SPRD_AUTO_CHASING_ENABLED, mCurrentEnableAutoChasing);
        }

        try {
            MeteringRectangle[] rect = new MeteringRectangle[mAutoChasingRegion.size()];
            if (mAutoChasingRegion.size()>0) {
                android.hardware.Camera.Area source = mAutoChasingRegion.get(0);
                if (source.rect.isEmpty()){
                    rect[0] = new MeteringRectangle(0,0,0,0,source.weight);
                } else {
                    rect = legacyAreasToMeteringRectangles(mAutoChasingRegion);
                }
            }
            requestSettings.set(CONTROL_SPRD_AUTO_CHASING_REGION,rect);
        } catch (Exception e) {
            Log.w(TAG, "" + e.getMessage());
        }

        if (mCurrentAppModeId == 1 /*camera_mode_manual*/) {
            requestSettings.set(LENS_FOCUS_DISTANCE, mFocusDistance);
        }

        if (mCurrentZoomRatio >= 0) {
            requestSettings.set(ANDROID_SPRD_ZOOM_RATIO,mCurrentZoomRatio);
            Log.i(TAG, " set ANDROID_SPRD_ZOOM_RATIO: mCurrentZoomRatio = "
                    + mCurrentZoomRatio);
        }

        updateExposureTime(requestSettings);

        Log.i(TAG,"set CONTROL_SPRD_PORTRAIT_OPTIMIZATION_ENABLED mPortraitOptEnable = " + mPortraitOptEnable);
        requestSettings.set(CONTROL_SPRD_PORTRAIT_OPTIMIZATION_ENABLED,mPortraitOptEnable);
        Log.i(TAG,"set CONTROL_SPRD_SMILE_CAPTURE mCurrentSmileCapture = " + mCurrentSmileCapture);
        requestSettings.set(CONTROL_SPRD_SMILE_CAPTURE,mCurrentSmileCapture);

        return requestSettings;
    }

    private void updateExposureTime(Camera2RequestSettingsSet requestSettings) {
        Integer aeMode = null;
        if (mCurrentAppModeId != 1 /*camera_mode_manual*/) {
            return;
        }
        if (mExposureTime != 0) {
            requestSettings.set(CONTROL_AE_MODE, CONTROL_AE_MODE_OFF);
            requestSettings.set(SENSOR_EXPOSURE_TIME, mExposureTime);
        } else {
            // Reset AE mode according to flash mode.
            ...
            requestSettings.set(CONTROL_AE_MODE, aeMode);
        }
    }
