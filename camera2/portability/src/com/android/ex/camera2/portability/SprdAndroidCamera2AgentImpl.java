...
import static android.hardware.camera2.CaptureResult.SCALER_CROP_REGION;
...
public class SprdAndroidCamera2AgentImpl extends AndroidCamera2AgentImpl {
    ...
    private int mLastFrameNumberOfFaces = -1;
...
//    private boolean mNeedAfBeforeCapture = true;
    ...
    SprdAndroidCamera2AgentImpl(Context context) {
        super(context);
        isNeedMoreLog = android.os.SystemProperties.getBoolean("persist.sys.cam.morelog",false);
        mSprdAgentImpl = this;
    ...
    public static class FaceDetectionCallbackForward implements CameraFaceDetectionCallback {
        ...
        @Override
        public void onFaceDetection(final Camera.Face[] faces, int[] attributes,final boolean faceChange) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mCallback.onFaceDetection(faces, attributes,faceChange);
                }
            });
        }
    }
    ...
    public static class AutoChasingTraceRegionCallbackForward implements CameraAutoChasingTraceRegionCallback {
        private final Handler mHandler;
        private final CameraAutoChasingTraceRegionCallback mCallback;

        public static AutoChasingTraceRegionCallbackForward getNewInstance(Handler handler, CameraAutoChasingTraceRegionCallback cb){
            if (handler == null || cb == null)
                return null;
            return new AutoChasingTraceRegionCallbackForward(handler,cb);
        }

        private AutoChasingTraceRegionCallbackForward(Handler handler, CameraAutoChasingTraceRegionCallback cb) {
            mHandler = handler;
            mCallback = cb;
        }
        ...
        public void onAutoChasingTraceRegion(int state, Rect rect) {
            mHandler.post(new Runnable() {
                public void run() {
                    mCallback.onAutoChasingTraceRegion(state,rect);
                }
            });
        }
    }

    public void monitorControlStateAutoChasingTraceRegion(CaptureResult result,
                                                     AndroidCamera2ProxyImpl camera2Proxy,
                                                          Rect activeArray){
        int[] autoChasingTraceRegion = result.get(CaptureResult.SPRD_AUTO_CHASING_TRACE_REGION);
        if (autoChasingTraceRegion == null || autoChasingTraceRegion.length < 3){
            return;
        }
        ...
        if (camera2Proxy instanceof  SprdAndroidCamera2ProxyImpl) {
            if (mCameraHandler instanceof SprdCamera2Handler) {
                SprdCamera2Handler handler = (SprdCamera2Handler) mCameraHandler;
                if (handler.getAutoChasingTraceRegionCallback() != null && autoChasingTraceRegion.length >= 3) {
                    Rect tmp = new Rect(autoChasingTraceRegion[1]-100,autoChasingTraceRegion[2]-100,autoChasingTraceRegion[1] + 100,autoChasingTraceRegion[2]+100);
                    Rect rect = faceForConvertCoordinate(activeArray, tmp);
                    if (autoChasingTraceRegion[1] == 0 && autoChasingTraceRegion[2] == 0 && rect != null) {
                        rect.setEmpty();
                    }
                    handler.getAutoChasingTraceRegionCallback().onAutoChasingTraceRegion(autoChasingTraceRegion[0],rect);
                }
            }
        }
    }
    ...
    public void monitorControlStatesAIDetect(CaptureResult result,
            ...
            Rect activeArray) {
        if (result.getFrameNumber() < 4) return; // Bug 1156421  work around for face detection report 0 at frame 0 , 1 ...
        ...
        if (faceState != null) {
            ...
            switch (faceStateMaybe) {
                case CaptureResult.STATISTICS_FACE_DETECT_MODE_SIMPLE:
                    ...
                    int[] attributes = null;
                    if (cameraProxy.getCapabilities().getSupportedFaceAttributesEnable()) {
                        attributes = result.get(CaptureResult.ANDROID_SPRD_FACE_ATTRIBUTES);
                    }
                    ...
                    Rect cropRect = result.get(SCALER_CROP_REGION);
                    /* @} */
                    for (int i = 0; i < faces.length; i++) {
                        Camera.Face face = new Camera.Face();
                        face.score = faces[i].getScore();
                        face.rect = faceForConvertCoordinate(cropRect, faces[i].getBounds());
                        cFaces[i] = face;
                    }
                    if (mCameraHandler instanceof SprdCamera2Handler) {
                        SprdCamera2Handler handler = (SprdCamera2Handler) mCameraHandler;
                        if (handler.getFaceDetectionListener() != null) {
                            handler.getFaceDetectionListener().onFaceDetection(cFaces, attributes,faceChange);
                        }
                    }
...
*/
    ...
    public Rect faceForConvertCoordinate(Rect cropRegion, Rect rect) {
        if (cropRegion == null || mPreviewSize == null) {
            return null;
        }
        int cropResizeWidth = cropRegion.width();
        int cropResizeHeight = cropRegion.height();
        float previewRation = (float) mPreviewSize.width()/mPreviewSize.height();
        float cropRation = (float) cropResizeWidth/cropResizeHeight;
        if (isNeedMoreLog) {
            Log.i(TAG, " mPreviewSize.width() = " + mPreviewSize.width() + " mPreviewSize.height() = " + mPreviewSize.height());
            Log.i(TAG, " cropResizeWidth = " + cropResizeWidth + " cropResizeHeight = " + cropResizeHeight + " hal rect :" + rect);
        }
        if (previewRation > cropRation) {
            cropResizeHeight = (int)(cropResizeWidth/previewRation );
        } else {
            cropResizeWidth = (int) (cropResizeHeight * previewRation);
        }
        /*
        int left = rect.left * 2000 / cropResizeWidth - 1000;
        int top = rect.top * 2000 / cropResizeHeight - 1000;
        int right = rect.right * 2000 / cropResizeWidth - 1000;
        int bottom = rect.bottom * 2000 / cropResizeHeight - 1000;*/

        int deltaCropX = Math.abs(cropResizeWidth - cropRegion.width());
        int deltaCropY = Math.abs(cropResizeHeight - cropRegion.height());
        RectF rectAfter = new RectF(rect);
        ...
        matrix.mapRect(rectAfter);
        int left = (int)rectAfter.left;
        int top = (int)rectAfter.top;
        int right = (int)rectAfter.right;
        int bottom = (int)rectAfter.bottom;
    ...
    public class SprdCamera2Handler extends AndroidCamera2AgentImpl.Camera2Handler {
        ...
        private AutoChasingTraceRegionCallbackForward mAutoChasingTraceRegionCallback;
        ...
        public void setAutoChasingTraceRegionListener(AutoChasingTraceRegionCallbackForward listener) {
            mAutoChasingTraceRegionCallback = listener;
        }

        public AutoChasingTraceRegionCallbackForward getAutoChasingTraceRegionCallback() {
            return mAutoChasingTraceRegionCallback;
        }
        ...
        public void handleMessage(final Message msg) {
            ...
            try {
                switch (cameraAction) {
                    ...
                    case SprdCameraActions.SET_AUTO_CHASING_LISTENER: {
                        setAutoChasingTraceRegionListener((AutoChasingTraceRegionCallbackForward) msg.obj);
                        break;//fix bug1158183
                    }
        ...
        protected void setPreviewTextureWithoutOptimize(SurfaceTexture surfaceTexture) {
            ...
            try {
                ...
                if(mCameraProxy.recordSurfaces != null &&mCameraProxy.recordSurfaces.size() > 0 ){
                    surfaceList.addAll(surfaceList);
                }
        ...
        protected void setRecoderRepeatingRequest() {
            ...
            try {
                mSession.setRepeatingRequest(
                        ...
                        //null, /*handler*/this);
                        mCameraResultStateCallback, /*handler*/this);
        ...
        protected void setPreviewDisplay(SurfaceHolder surfaceHolder) {
            ...
            if(mCameraProxy.surfacesChanged){
                setPreviewDisplayWithoutOptimize(surfaceHolder);
...
//            outConfigurations.add(new OutputConfiguration(mPreviewSurface, mPreviewSize.width(), mPreviewSize.height()));
            ...
            if(mCameraProxy.recordSurfaces != null){
                for (Surface recordSurface : mCameraProxy.recordSurfaces){
                    Log.w(TAG,"setPreviewDisplay add surface " + recordSurface);
...
//            outConfigurations.add(new OutputConfiguration(mPreviewSurface, mPreviewSize.width(), mPreviewSize.height()));
        ...
        protected void onMonitorControlStates(CaptureResult result) {
            ...
            if (isNeedMoreLog) {
                Log.i(TAG,"CaptureResult CONTROL_AF_STATE="+result.get(CaptureResult.CONTROL_AF_STATE));
                Log.i(TAG,"CaptureResult CONTROL_AE_STATE="+result.get(CaptureResult.CONTROL_AE_STATE));
                Log.i(TAG,"CaptureResult STATISTICS_FACE_DETECT_MODE="+result.get(CaptureResult.STATISTICS_FACE_DETECT_MODE));
                Log.i(TAG,"CaptureResult CONTROL_SPRD_BLUR_COVERED="+result.get(CaptureResult.CONTROL_SPRD_BLUR_COVERED));
                Log.i(TAG,"CaptureResult ANDROID_SPRD_IS_HDR_SCENE="+result.get(CaptureResult.ANDROID_SPRD_IS_HDR_SCENE));
                Log.i(TAG,"CaptureResult CONTROL_SPRD_AI_SCENE="+result.get(CaptureResult.CONTROL_SPRD_AI_SCENE));
                Log.i(TAG,"CaptureResult CONTROL_SPRD_BLUR_CAPTURE="+result.get(CONTROL_SPRD_BLUR_CAPTURE));
                Log.i(TAG,"CaptureResult CONTROL_SPRD_NEED_AF_BEFORE_CAPTURE="+result.get(CaptureResult.CONTROL_SPRD_NEED_AF_BEFORE_CAPTURE));
                Log.i(TAG,"CaptureResult SPRD_AUTO_CHASING_TRACE_REGION ChasingRegion="+result.get(CaptureResult.SPRD_AUTO_CHASING_TRACE_REGION));
            }
        }
    }
    ...
    protected class SprdAndroidCamera2ProxyImpl extends
        ...
        @Override
        public void setPreviewCallbackConfig(boolean config) {
            mConfigPreivewCallback = config;
        }
            ...
            if (jpeg != null) {
                ...
                if (planeList.length == 1) {
                    if (mLastSettings.getNormalHdrModeEnable() == 1) {
                        ...
                        if (getCameraState().getState() >= AndroidCamera2StateHolder.CAMERA_PREVIEW_READY)
                            changeState(mLastCameraState);
                    }
        ...
        public void stopPreviewWithAsync() {
            ...
            if (getCameraState().isInvalid()) {
                Log.e(TAG, "stopPreviewWithAsync return");
                return;
            }
            try {
                getDispatchThread().runJob(new Runnable() {
        ...
        @Override
        public void setAutoChasingTraceRegionCallback(Handler handler, CameraAutoChasingTraceRegionCallback callback) {
            mCameraHandler.obtainMessage(
                    SprdCameraActions.SET_AUTO_CHASING_LISTENER,
                    AutoChasingTraceRegionCallbackForward.getNewInstance(handler, callback)).sendToTarget();
        }
...
//        saveBytes(data, "yuv");
    ...
    private static byte[] getNV21FromImage(Image img) {
        ...
        buf.clear();
        Log.v(TAG, "getNV21FromImage cost " + (System.currentTimeMillis() - t1));
        return imageBytes;
    }
