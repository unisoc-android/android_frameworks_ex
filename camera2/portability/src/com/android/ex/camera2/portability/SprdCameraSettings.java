...
//    <integer name="camera_mode_3dnr_video">21</integer>
    ...
    protected int mCurrentEnableLogoWatermark = -1;
    protected int mCurrentEnableTimeWatermark = -1;
    ...
    protected final List<Camera.Area> mAutoChasingRegion = new ArrayList<>();
    protected final List<Camera.Area> mTouchAreas = new ArrayList<>();

    protected static final int[] AUTO_CHASING_DEFAULT_VALUE = new int[]{0,0};

    protected float mZoomMin = 1.0f;

    protected int mPortraitOptEnable = 1;
    ...
    protected SprdCameraSettings(SprdCameraSettings src) {
        ...
        mCurrentEnableLogoWatermark = src.mCurrentEnableLogoWatermark;
        mCurrentEnableTimeWatermark = src.mCurrentEnableTimeWatermark;
        ...
        mAutoChasingRegion.addAll(src.mAutoChasingRegion);
        mCurrentEnableAuto3Dnr = src.mCurrentEnableAuto3Dnr;

        mCurrentEnableFaceAttributes = src.mCurrentEnableFaceAttributes;
        mTouchAreas.addAll(src.mTouchAreas);
        mPortraitOptEnable = src.mPortraitOptEnable;
        mCurrentEnableAutoChasing = src.mCurrentEnableAutoChasing;
        mZoomMin = src.mZoomMin;
        mCurrentSmileCapture = src.mCurrentSmileCapture;
        mCurrentCallbackSize = src.mCurrentCallbackSize;
    }
    ...
    public void setLogoWatermarkEnable(int enable) { mCurrentEnableLogoWatermark = enable; }
    public void setTimeWatermarkEnable(int enable) { mCurrentEnableTimeWatermark = enable; }
    ...
    public void setDefault(boolean isLcd, CameraCapabilities cameraCapabilities) {
        ...
        setCurrentAutoChasingRegion(null);
        setPortraitOptEnable(0);
        setSmileCapture(0);
    }
    /* @} */
    ...
    public void setCurrentAutoChasingRegion(List<Camera.Area> autoChasingRegion){
        mAutoChasingRegion.clear();
        if (autoChasingRegion != null)
        mAutoChasingRegion.addAll(autoChasingRegion);
    }
    ...
    public void setTouchAreas(List<Camera.Area> areas){
        mTouchAreas.clear();
        if (areas != null) {
            mTouchAreas.addAll(areas);
        }
    }

    public List<Camera.Area> getTouchAreas() {
        return new ArrayList<Camera.Area>(mTouchAreas);
    }

    public void setCurrentFaceAttributes(int enable){
        mCurrentEnableFaceAttributes = enable;
    }
    /* @} */

    public void setPortraitOptEnable(int portraitOptEnable) {
        mPortraitOptEnable = portraitOptEnable;
    }

    public int getPortraitOptEnable() {return mPortraitOptEnable;}

    public int mCurrentEnableAutoChasing = -1;

    public void setAutoChasingEnable(int enable) {
        mCurrentEnableAutoChasing = enable;
    }

    public int getAutoChasingEnable(){
        return mCurrentEnableAutoChasing;
    }

    public void setZoomMin(float zoomMin){mZoomMin = zoomMin;}

    public float getZoomMin() {return mZoomMin;}

    protected Size mCurrentCallbackSize;

    public void setCallbackSize(Size callbackSize) {
        mCurrentCallbackSize = new Size(callbackSize);
    }

    public Size getCurrentCallbackSize() {
        return new Size(mCurrentCallbackSize);
    }

    public int mCurrentSmileCapture = 0;

    public void setSmileCapture(int enable) {
        mCurrentSmileCapture = enable;
    }

    public int getSmileCapture(){
        return mCurrentSmileCapture;
    }
}
