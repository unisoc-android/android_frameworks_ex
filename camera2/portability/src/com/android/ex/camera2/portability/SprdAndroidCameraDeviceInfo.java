...
public abstract class SprdAndroidCameraDeviceInfo implements CameraDeviceInfo {
    ...
    public int getTruthCameraId(int cameraId) {
        ...
        if (SPRD_3D_VIDEO_ID == cameraId) {
            ...
            cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        } else if (SPRD_BACK_TRI_ID == cameraId) {
	    cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        } else if(SPRD_BACK_PORTRAIT_ID == cameraId){
            cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        } else if (SPRD_BACK_HIRES_ID == cameraId){
            cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        } else if (SPRD_FRONT_HIRES_ID == cameraId){
            cameraId = Camera.CameraInfo.CAMERA_FACING_FRONT;
        } else if (SPRD_BACK_IR_ID == cameraId){
            cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        } else if (SPRD_BACK_MACRO_ID == cameraId){
            cameraId = Camera.CameraInfo.CAMERA_FACING_BACK;
        }
        /* @} */
        return cameraId;
